...
nb_drivers = 600000; #nb of travels for clustering
nb_drivers_choices = 600000; #nb of travels for choosing the PCA components and K numbers
...
if (not GROUP_BY_ID):
    selected_Features.append('fix_speed_cnt_65');
    selected_Features.append('fix_speed_dur_65');
    print('clustering on the travels (not on the drivers)');
else:
    print('clustering on the drivers (not on the drivers)');
...
def getdata(file_name, selected_features, nb_drivers):
    ...
    with open(file_name) as csv_file:
               ...
               float(row['max_speed']) < 120):
                ...
                for feature in selected_features:
                    if (feature in distance_features):
                        line.append(float(row[feature])/distance);
                ...
                if(float(row['crash_cnt'])>0):
                    crash_cnt += float(row['crash_cnt']);
        ...
        print(crash_cnt, 'accidents have been detected for those trips')
    data = np.array(data);
    if(GROUP_BY_ID):
        data2 = group_by_id(data);
        l_id = data2[:,0];
        for g in range(len(l_id)):
            if (l_id[g] in l_id[g+1:]):
                print("Twice the same driver id !!!!!!!!!");
                return 0,0;
        print("check : each driver id is unique");
    else:
        data2 = data;
    ...
    if(not INCLUDE_CRASHES):
        ...
        crashes = ['no need for crashes if we take them into account in the clustering'];
    return data2, crashes;  
...
K_CHOICE ):
    ...
    data_choices = np.array(StandardScaler().fit_transform(data_choices));
...
if(PCA_RESULTS):
    ...
    if(CLUSTER_COMPOSITION):
        can.drawString(10, 160, "Population of the clusters: " );
        can.drawString(0, 150,str(occ));
    can.save();
